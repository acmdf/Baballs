...
{
    Model3D* model = GetModel(modelId);
    if (!model || model->indexCount == 0) return;
...
{
    if (!model || model->blendShapes.empty()) return;
...
{
    if (model->currentVertices.size() != blendShape.targetVertices.size()) return;
...
{
    if (!model || model->vertexBuffer == 0 || model->currentVertices.empty()) return;
...
void ModelManager::UpdateAnimation(Model3D* model, float deltaTime)
{
    ...
    if (!model || !model->isAnimating || model->currentAnimation.empty()) return;
